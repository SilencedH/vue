<template>
  <v-card class="mx-auto hide-overflow" style="max-width: 600px;">
    <v-layout>
      <v-flex xs6 d-flex>
        <v-img src="https://cdn.vuetifyjs.com/images/ratings/fortnite1.png"></v-img>
      </v-flex>
      <v-flex xs6>
        <v-container grid-list-md pa-0 pl-2 style="margin: -4px 0">
          <v-layout wrap>
            <v-flex xs7 d-flex>
    ...
    <v-card-title class="align-start">
      ...
      <v-dialog v-model="dialog" width="400">
        ...
        <v-card>
          ...
          <v-text-field ref="link" :label="copied ? 'Link copied' : 'Click to copy link'" class="pa-3" readonly value="https://g.co/kgs/nkrK43" @click="copy"></v-text-field>
        </v-card>
      </v-dialog>
    </v-card-title>
    <v-divider></v-divider>
    <v-card-actions>
      ...
      <v-rating v-model="rating" length="10" readonly>
        <v-icon slot="item" slot-scope="props" :color="props.isFilled ? 'purple darken-4' : ''" v-text="`mdi-numeric-${props.index}-box`"></v-icon>
      </v-rating>
    </v-card-actions>
