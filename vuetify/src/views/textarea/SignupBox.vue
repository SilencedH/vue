<template>
  <v-card class="mx-auto" style="max-width: 500px;">
    ...
    <v-form ref="form" v-model="form" class="pa-3 pt-4">
      <v-text-field v-model="password" :rules="[rules.password, rules.length(6)]" box color="deep-purple" counter="6" label="Password" style="min-height: 96px" type="password"></v-text-field>
      <v-text-field v-model="phone" box color="deep-purple" label="Phone number" mask="phone"></v-text-field>
      <v-text-field v-model="email" :rules="[rules.email]" box color="deep-purple" label="Email address" type="email"></v-text-field>
      <v-textarea v-model="bio" auto-grow box color="deep-purple" label="Bio" rows="1"></v-textarea>
    ...
    <v-card-actions>
      ...
      <v-spacer></v-spacer>
      <v-btn :disabled="!form" :loading="isLoading" class="white--text" color="deep-purple accent-4" depressed>Submit</v-btn>
    </v-card-actions>
